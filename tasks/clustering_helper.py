    ...
    try:
        ...
        for label_val, songs_list in filtered_clusters.items():
            if songs_list: 
                ...
                # Create a unique key for this playlist to prevent merging song lists
                playlist_key = base_name
                # If this base_name has been generated more than once (for different raw clusters), make the key unique.
                # label_val is the raw cluster ID, unique within this iteration.
                if raw_cluster_base_name_generation_count[base_name] > 1:
                    playlist_key = f"{base_name} (RawCluster-{label_val})"
        ...
        # Log if any base names were generated by multiple raw clusters
        for name, count in raw_cluster_base_name_generation_count.items():
            if count > 1:
                # Note: current_named_playlists[name] might not exist if all occurrences were disambiguated.
                print(f"{log_prefix} Iteration {run_idx}: Playlist base name '{name}' was generated by {count} raw clusters. They are stored as distinct playlists (e.g., '{name} (RawCluster-ID)').")
